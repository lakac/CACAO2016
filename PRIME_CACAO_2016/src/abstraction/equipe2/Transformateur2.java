...
public class Transformateur2 implements Acteur, ITransformateur{
	...
	public static double[] CoutInts (double p, double []T){ 
		...
		CI[1] = CI[0]*0.6/T[1];
		// 600g de cacao 駲uivalent � 1kg de chocolat
		return CI;
		//Test OK
	}
	...
	//la quantit� demand馥 aux producteurs est proportionnelle 

	
	// Quantit� annonc馥 aux producteurs 
	...
	//Le prix du kilo de chocolat 騁ant fix�, tout ce que l'on peut calculer c'est la marge que l'on se fait.
	...
	//M騁hode principale de test de CoutInts, d馭饌ire les "/*" pour l'activer
	public void next() {}
		//setT(qdd);
		...
		System.out.println("le cout de revient de Nestl� France � la p駻iode t est de "+CI[0]);
		System.out.println("le cout de revient unitaire de Nestl� France � la p駻iode t est de "+CI[1]);
		...
		System.out.println("le b駭馭ice fait a cet step est de :" + Benefice(T,prixdevente,p) + "�");
		

	/*	if (stock_cacao(T,S1)<0){

			System.out.println("Erreur dans le syst鑪e");
		}else{
		...
		for(int i=0;i<4;i++){
			if (T[i]<0){
				...
				if (T[i]==0){
					System.out.println("Gr钁e des distributeurs");
				}else{
		...
		//Pour nous permettre de savoir si on a mit le bon prix pour les co皦s d'achats du cacao et de vente du chocolat
