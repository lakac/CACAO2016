...
public class MarcheConsommateur implements Acteur {
	...
	public MarcheConsommateur(){
		MarcheConsommateur.distributeurs=new ArrayList<IDistributeur>();
	...
	public static void ajouterDistributeur(IDistributeur distributeur){
		MarcheConsommateur.distributeurs.add(distributeur);
	}//  
	...
	public void actualiserFidelite(){
		for (Produit p : cata.getProduits()){
			//if Carrefour et Leclerc sont en concurrence sur ce produit/) (V3)
				if ((MarcheConsommateur.distributeurs.get(1).getPrixVente(p)>MarcheConsommateur.distributeurs.get(0).getPrixVente(p))&&(this.fidelite.get("Carrefour").get(p)>FIDELITE_MIN)){//si prix carrefour superieur
						this.fidelite.get(MarcheConsommateur.distributeurs.get(0)).put(p,this.fidelite.get(MarcheConsommateur.distributeurs.get(0)).get(p)+VARIATION_FIDELITE);
						this.fidelite.get(MarcheConsommateur.distributeurs.get(1)).put(p,this.fidelite.get(MarcheConsommateur.distributeurs.get(1)).get(p)-VARIATION_FIDELITE);
					}
					if ((MarcheConsommateur.distributeurs.get(1).getPrixVente(p)<MarcheConsommateur.distributeurs.get(0).getPrixVente(p))&&(this.fidelite.get("Leclerc").get(p)>FIDELITE_MIN)){//si prix carrefour superieur
						this.fidelite.get(MarcheConsommateur.distributeurs.get(0)).put(p,this.fidelite.get(MarcheConsommateur.distributeurs.get(0)).get(p)-VARIATION_FIDELITE);
						this.fidelite.get(MarcheConsommateur.distributeurs.get(1)).put(p,this.fidelite.get(MarcheConsommateur.distributeurs.get(1)).get(p)+VARIATION_FIDELITE);
					}
			}	
	...
	public void initialiserFidelite(){
		for (Produit p : cata.getProduits()){
			this.fidelite.get(MarcheConsommateur.distributeurs.get(0)).put(p, 0.3);
			this.fidelite.get(MarcheConsommateur.distributeurs.get(1)).put(p, 0.3);
			for (IDistributeur d : MarcheConsommateur.distributeurs){
				if(!(d.equals(MarcheConsommateur.distributeurs.get(1))&&d.equals(MarcheConsommateur.distributeurs.get(0)))){
