...
public class Producteur implements Acteur,IProducteur{
	...
	private Indicateur prodUtilisable; //production totale sans les pertes
    ...
    public Producteur(Monde monde) {
       ...
       this.prodUtilisable = new Indicateur(Constantes.IND_PRODUCTION_P2, this, 0.0);
       ...
       for (Acteur a : Monde.LE_MONDE.getActeurs()) {
    	...
    	Monde.LE_MONDE.ajouterIndicateur( this.prodUtilisable );
	...
	public void next(){
		...
		if (Monde.LE_MONDE.getStep()%12==1){ //production semi annuelle
			...
			this.prodUtilisable.setValeur(this,this.prod-this.pertes.getValeur());
			this.Augment_Stock(this.prodUtilisable.getValeur());
			this.journal.ajouter("Production de semi annuelle de " + this.prodUtilisable);
		}
	...
	public double annonceQuantiteMiseEnVente(ITransformateur t) {
		return (this.prodUtilisable.getValeur()/24);
	}
