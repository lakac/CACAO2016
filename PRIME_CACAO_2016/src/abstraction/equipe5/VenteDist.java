...
public class VenteDist {
	...
	public List<CommandeDistri> offre(List<CommandeDistri> listeCommandesDist){
		...
		for(int i=0; i<lindt.getDistributeurs().size(); i++){
			...
			double stockChocolatI=0.25*(lindt.getStocksChocolat().get(i).getStock()); //stock de chocolat i disponible pour Leclerc+Carrefour (25%), on se reserve un stock minimal
